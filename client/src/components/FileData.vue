  ...
  <div class="container-fluid mybody">
    <div v-if="xlsxData">
      ...
      <div class="row">
        <div class="col-12">
          <div
            class="table-container scrollable-table"
            ref="tableBody"
            @scroll="scrollBoth('body')"
          >
            <table class="table">
              <thead ref="tableHead" @scroll="scrollBoth('head')">
                <tr>
                  <th v-for="(column, index) in xlsxData[0]" :key="index">
                    ...
                    />
                    <p v-else @click="startEditing(0, index)">
                      {{ xlsxData[0][index] }}
                    </p>
                  </th>
              ...
              <tbody>
                <tr v-for="(item, rowIndex) in displayedItems" :key="rowIndex">
                  <td v-for="(value, colIndex) in item" :key="colIndex" style="width: 300px;">
                    <input
                      v-if="isEditing(rowIndex + 1, colIndex)"
                      v-model="item[colIndex]"
                      ref="editingInput"
                    />
                    <p v-else @click="startEditing(rowIndex + 1, colIndex)">
                      {{ value !== "" ? value : "-" }}
                    </p>
                    <!-- <div v-if="isEditing(rowIndex + 1, colIndex)" class="data">
                      <input v-model="item[colIndex]" ref="editingInput" />
                    </div>
                    <div
                      v-else
                ...
                <tr
                  v-if="
                    ...
                    (totalPages === 0 && xlsxData.length > 0)
                  "
                >
                  <button
                    class="btn btn-outline-primary newmargin"
                    @click="addNew()"
                  >
                    <img src="../assets/add.png" />
                  </button>
    ...
    <div>
      <button
        class="btn btn-outline-primary mymargin1"
        @click="saveXLS"
        v-if="xlsxData"
      >
        Save
  ...
  >
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">File Saved Successfully</div>
